PRAYER_CALCULATION_JURISTIC_METHOD_STRINGS = [
    "Jafari - Ithna Ashari",
    "Karachi - University of Islamic Sciences",
    "ISNA - Islamic Society of North America",
    "MWL - Muslim World League",
    "Mecca - Umm al-Qura",
    "Egyptian General Authority of Survey",
    "Custom Setting",
    "University of Tehran - Institute of Geophysics",
    "Algerian Minister of Religious Affairs and Wakfs",
    "Gulf 90 Minutes Fixed Isha",
    "Egyptian General Authority of Survey (Bis)",
    "UOIF - Union Des Organisations Islamiques De France",
    "Sistem Informasi Hisab Rukyat Indonesia",
    "Diyanet İşleri Başkanlığı",
    "Germany Custom",
    "Russia Custom",
]

ASR_JURISTIC_METHOD_STRINGS = ["Standard (Shafi, Hanbli, Maliki)", "Hanafi"]

HIGH_LATITUDES_STRING = ["None", "Midnight", "1/7th", "Angle"]

TIME_FORMAT_STRINGS = [
    "24 Hour Format",
    "12 Hour Format",
    "12 Hour Format w/o Suffix",
    "Floating Number",
]

MAGHRIB_RULE_STRINGS = ["Angle", "Fixed Minutes"]

ISHA_RULE_STRINGS = ["Angle", "Fixed Minutes"]

PRAYER_TIMES_URL = "http://www.islamicfinder.us/index.php/api/prayer_times"

GEOLOCATION_URL = "https://freegeoip.app/json/"

HIJRI_DATE_URL = "http://www.islamicfinder.us/index.php/api/calendar"

HIJRI_MONTH_STRING = {
    1: "Muharram",
    2: "Safar",
    3: "Rabi' al-awwal",
    4: "Rabi' al-thani",
    5: "Jumada al-ula",
    6: "Jumada al-akhirah",
    7: "Rajab",
    8: "Sha'ban",
    9: "Ramadan",
    10: "Shawwal",
    11: "Dhu al-Qidah",
    12: "Dhu al-Hijjah",
}


WEATHER_API_LOCATION_SEARCH_URL = "https://www.metaweather.com/api/location/search/"

WEATHER_API_URL = "https://www.metaweather.com/api/location/"

WEATHER_API_ICONS_BASE_URL = "https://www.metaweather.com/static/img/weather/png/64/"

MOON_API_URL = "https://www.icalendar37.net/lunar/api/"

CONFIG_THEME_SECTION = "THEME"
CONFIG_ALARM_SECTION = "ALARMS"
CONFIG_MISC_SECTION = "MISC"

CONFIG_ENABLE_DARK_MODE_KEY = "ENABLE_DARK_MODE"
CONFIG_FAJR_ALARM_KEY = "FAJR_ALARM"
CONFIG_TAHAJJUD_ALARM_KEY = "TAHAJJUD_ALARM"
CONFIG_USE_FAHRENHEIT_KEY = "USE_FAHRENHEIT"
CONFIG_USE_HANAFI_METHOD_KEY = "USE_HANAFI_METHOD"

LIGHT_THEME_TEXT_COLOR = [0.65, 0.7, 0.01, 1]
LIGHT_THEME_CLOCK_COLOR = [0, 0, 0, 1]

DARK_THEME_TEXT_COLOR = [0.61, 0.81, 0.01, 0.76]
DARK_THEME_CLOCK_COLOR = [1, 1, 1, 1]
